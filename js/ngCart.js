...
angular.module('ngCart', ['ngCart.directives'])
    ...
    .service('ngCart', ['$rootScope', 'ngCartItem', 'store', function ($rootScope, ngCartItem, store) {
        ...
        this.addItem = function (id, name, price, quantity, data) {
            var elementCart = document.getElementsByClassName('addCartItem')[0];

            var inCart = this.getItemById(elementCart.id);

            if (typeof inCart === 'object') {
                //Update quantity of an item if it's already in the cart
                inCart.setQuantity(quantity, false);
            } else {
                var newItem = new ngCartItem(elementCart.id, elementCart.getAttribute("name"), elementCart.getAttribute("price"), quantity, data);
